<?php
/**
 * One-Click Live Updater: qris_payment.blade.php & routes/web.php
 * Akses: https://app.mesinbayar.com/fhk/update_kiosk.php
 */

$qrisPaymentContent = <<<'BLADE'
            ...
            <div class="space-y-5">
                ...
                <div class="space-y-3.5 text-xs">
                    ...
                    <div class="flex justify-between items-center pb-1">
                        ...
                        <span class="font-bold text-white">{{ $transaksi->kiosk->name ?? $transaksi->kiosk_id ?? 'Kios FHK' }}</span>
                    </div>
                </div>

                <!-- Total Pembayaran -->
                <div class="p-4 rounded-2xl bg-slate-950/80 border border-slate-800 space-y-1 text-center">
                    <span class="text-xs text-slate-400 uppercase tracking-wider font-semibold">Total yang Harus Dibayar:</span>
                ...
                <!-- Petunjuk Singkat -->
                <div class="p-3.5 rounded-xl bg-cyan-950/30 border border-cyan-800/40 text-xs text-slate-300 space-y-1">
...
$targetView = __DIR__ . '/resources/views/kiosk/qris_payment.blade.php';
$writtenView = @file_put_contents($targetView, $qrisPaymentContent);
...
<head>
    <meta charset="UTF-8">
    <title>Kiosk Updater Sukses</title>
    <style>body{background:#0f172a;color:#f8fafc;font-family:system-ui;padding:40px;text-align:center;}</style>
</head>
<body>
    <div style="max-width:500px;margin:0 auto;background:#1e293b;padding:30px;border-radius:16px;border:1px solid #334155;">
        <h2 style="color:#38bdf8;">✅ Update Kios Berhasil!</h2>
        <p style="color:#94a3b8;font-size:14px;">File <code>qris_payment.blade.php</code> berhasil ditulis (<?= $writtenView ?> bytes) dan <?= $del ?> cache view dibersihkan.</p>
        <p><a href="kiosk/qris/S1ZfNPCCouJwrgDEV7t7" style="display:inline-block;padding:10px 20px;background:#06b6d4;color:#0f172a;text-decoration:none;border-radius:8px;font-weight:bold;">Buka Halaman Invoice QRIS</a></p>
    </div>
</body>
</html>
